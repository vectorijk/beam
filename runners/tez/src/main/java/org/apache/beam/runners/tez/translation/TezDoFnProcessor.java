...
import org.apache.beam.runners.tez.NoOpStepContext;
  ...
  public void run() throws Exception {
    ...
    if (kvsReader != null) {
      while (kvsReader.next()) {
        outputManager.setCurrentElement(
            WindowedValue.valueInGlobalWindow(
                TranslatorUtil.convertToJavaType(kvsReader.getCurrentKey())));
      ...
      while (kvReader.next()) {
        WindowedValue value =
            WindowedValue.valueInGlobalWindow(
                TranslatorUtil.convertToJavaType(kvReader.getCurrentKey()));
        outputManager.setCurrentElement(value);
        runner.processElement(
            WindowedValue.valueInGlobalWindow(
                TranslatorUtil.convertToJavaType(kvReader.getCurrentValue())));
      }
    } else {
      throw new RuntimeException("UNSUPPORTED READER!");
    }
