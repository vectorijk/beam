...
public class OrcFileGenerator {
  ...
  /**
   * Writes an ORC file with nested composite types and null values on different levels. Generates
   * {@link OrcRowInputFormatTest#TEST_FILE_COMPOSITES_NULLS}.
   */
          ...
          if (i != 2) {
            ...
            for (int j = 0; j < mapSize; j++) {
              ...
              if (j != 1) {
                ...
                keys.setVal(
                    mapEntryOffset, ("key-" + row + "-" + j).getBytes(StandardCharsets.UTF_8));
              } else {
                keys.isNull[mapEntryOffset] = true;
              }
              if (j != 2) {
                // value in third map entry is always null
                vals.vector[mapEntryOffset] = (long) row + i + j;
              } else {
  ...
  /**
   * Writes an ORC file with nested composite types and repeated values. Generates {@link
   * OrcRowInputFormatTest#TEST_FILE_REPEATING}.
   */
